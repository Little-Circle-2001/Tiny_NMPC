function [PIPG_x_output, PIPG_u_output, PIPG_lambda_output, IterNum] = xPIPG_Solver(H, h, G, g, x0, u0, lambda0, Scl_Mat, ...
                                                                          PIPG_MaxIter, PIPG_Tol, alpha, beta, ...
                                                                          PIPG_z_lower, PIPG_z_upper)
    % 状态量x的维度为(x_dim, N+1)
    % 控制量u的维度为(u_dim, N)
...
%     tic;
    while p <= PIPG_MaxIter
        ...
        z1 = xi0 - alpha * (H * xi0 + h + G' * eta0);
        z1 = min(max(PIPG_z_lower,z1),PIPG_z_upper);

        lambda_vec1 = eta0 + beta * (G * (2 * z1 - xi0) - g);

        xi1 = (1-rho) * xi0 + rho * z1;

        eta1 = (1-rho) * eta0 + rho * lambda_vec1;
