function [PIPG_x_output, PIPG_u_output, PIPG_lambda_output] = PIPG_Solver_OptimalStep(H, h, G, g, x0, u0, lambda0, ...
                                                                          PIPG_MaxIter, PIPG_Tol, alpha, beta)
    % 状态量x的维度为(x_dim, N+1)
    % 控制量u的维度为(u_dim, N)
    % 动力学约束拉格朗日乘子λ的维度为(x_dim, N)
    % 
    % 此版本使用Alg.1a的步长公式计算最优步长，用于验证步长选择对收敛速度的影响
    ...
    % 如果提供了alpha和beta，可以选择使用提供的值或最优值
    % 这里使用最优值
    alpha = alpha_optimal;
    beta = beta_optimal;
